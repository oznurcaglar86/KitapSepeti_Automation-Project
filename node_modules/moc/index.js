...
var staticExtensions = ['html', 'js', 'css']
...
function staticServe (staticConf) {
    const serve = require('koa-static')
    return serve(
        staticConf.direction,
        {
            extensions: staticConf.extensions || staticExtensions
        }
    )
}
...
exports.start = function (config) {
    ...
    if (config.static) {
        plugins.push({
            handler: staticServe,
            options: config.static
        })
    }

    if (config.plugins) {
        for (const key in config.plugins) {
            plugins.push(config.plugins[key])
        }
    }
    ...
    plugins.forEach(function (plugin) {
        if (typeof plugin === 'function') {
            ...
            app.use(plugin.handler(plugin.options || {}))
        }
    })
